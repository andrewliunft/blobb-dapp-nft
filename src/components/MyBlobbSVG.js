...
function MyBlobbSVG({ currAccount, blobb, show, setShow }) {
  //start: 255 0 208 -- end: 34 148 165
  //start: "253,10,192", end: "31,0,167" trailer color
  // const [state, setState] = useState({exp: 0, lvl: 1, time: 100, colors: {...blobb.colors, ...{a: 0.3}}})

  // setTimeout(() => {
  //   if(state.lvl === 99) return
  //   let exp = state.exp + 1
  //   if(state.exp === 9) {
  //     if(state.lvl === 98) setState({...state, ...{exp: 10, lvl: 99, colors: {...state.colors, a: 1}}})
  //     else {
  //       let lvl = state.lvl +1
  //       setState({...state, ...{exp: 0, lvl, time: Math.max(5, state.time)}})
  //     }
  //   }
  //   else setState({...state, ...{exp, lvl: state.lvl, time: state.time}})
  // }, state.time)

  // setTimeout(() => {
  //   const v = state.colors <= .1 ? 0.1 : -0.1
  //   const a = state.colors.a+0.05
  //   setState({...state, ...{time: 250, colors: { ...state.colors, a }}})
  // }, state.time);

  // setTimeout(() => {
  //   let start = Math.round(Math.random()*255)+", "+Math.round(Math.random()*255)+", "+Math.round(Math.random()*255)
  //   let end = Math.round(Math.random()*255)+", "+Math.round(Math.random()*255)+", "+Math.round(Math.random()*255)
  //   setState({...state, ...{colors: { start, end }}})
    ...
    <svg className={show ? classes.blobb_svg_w_stats : classes.blobb_svg } xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" fill="none">
      ...
      <defs>
        ...
        <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style={{stopColor: "rgb("+blobb.colors.start+","+blobb.hp/10+")"}}>
            {blobb.bType === 4 && 
              <animate 
                attributeName="stop-color" 
                values={"rgba(255,0,0,"+ blobb.hp/10 +");rgba(0,255,0,"+ blobb.hp/10 +");rgba(0,0,255,"+ blobb.hp/10 +");rgba(255,0,0,"+ blobb.hp/10 +");"}
                dur="5s" 
                repeatCount="indefinite" 
              />
            }
          </stop>
          <stop offset="100%" style={{stopColor: "rgb("+blobb.colors.end+","+blobb.hp/10+")"}}>
            {blobb.bType === 4 && 
              <animate 
                attributeName="stop-color" 
                values={"rgba(255,255,0,"+ blobb.hp/10 +");rgba(0,255,255,"+ blobb.hp/10 +");rgba(255,0,255,"+ blobb.hp/10 +");rgba(255,255,0,"+ blobb.hp/10 +");"}
                dur="5s" 
                repeatCount="indefinite" 
              />
            }
        ...
        <linearGradient id="s-grad" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" style={{stopColor: "rgb("+blobb.colors.start+")"}}>
            {blobb.bType === 4 && 
              <animate 
                attributeName="stop-color" 
          ...
          <stop offset="100%" style={{stopColor: "rgb("+blobb.colors.end+")"}}>
            {blobb.bType === 4 && 
              <animate 
                attributeName="stop-color" 
          ...
          {blobb.bType >= 2 && <animateTransform attributeName="gradientTransform" type="rotate" values="0 .5 .5;360 .5 .5" dur="1.5s" repeatCount="indefinite" />}
        </linearGradient>
  ...
  //       <circle r={10} fill="url(#grad)" stroke="url(#s-grad)" strokeWidth={1} filter="drop-shadow(1 1 1 #000b)" />
  //       <circle r={10} stroke="url(#s-grad)" strokeWidth={2} filter="blur(1px)" />
  //       <circle id="exp" r="15" stroke={blobb.number <= 10 ? "url(#c-grad)" : "url(#t-grad)"} strokeDasharray="10" strokeDashoffset={(blobb.totalAttacks < MAX_ATTACKS_LVL ? 10 - state.exp /* blobb.totalAttacks%10 */ : 10)} pathLength="10" strokeWidth="2" strokeLinecap="round" filter="drop-shadow(0 0 1 #000b)" transform="rotate(-90)" />
  //       <text fill={blobb.number <= 10 ? "url(#c-grad)" : "url(#t-grad)"} filter="drop-shadow(0 1px 0 #000d)" textAnchor="middle" style={{fontFamily: "Titan One", pointerEvents: "none", fontSize: "12px"}} transform="translate(0 4)">
  //         {blobb.totalAttacks < MAX_ATTACKS_LVL ? state.lvl /*Math.floor(blobb.totalAttacks/10)*/ : 99}
  //       </text>
  //     </g>
  ...
  //       />
  //       <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
  //         <stop offset="0%" style={{stopColor: "rgb("+state.colors.start+","+state.colors.a+")"}} />
  //         <stop offset="100%" style={{stopColor: "rgb("+state.colors.end+","+state.colors.a+")"}} />
  //       </linearGradient>
  //       <linearGradient id="s-grad" x1="0%" y1="0%" x2="100%" y2="0%">
  //         <stop offset="0%" style={{stopColor: "rgb("+state.colors.start+")"}} />
  //         <stop offset="100%" style={{stopColor: "rgb("+state.colors.end+")"}} />
